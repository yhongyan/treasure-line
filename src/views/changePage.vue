	...
	<div class="content-change row clear">
			...
			<div class="left-top">
				<div class="now-up">
					...
					<div v-for="(item,index) in jiaoyidui" @click="upType=index+1">
						<el-row class="up-item" :style="upType==index+1?'border-left: 3px solid #f07e1b;background: rgb(255,246,238)':''">
		...
		<div class="main-right col-lg-9 col-md-8 col-sm-12">
					...
					<div class="buy-recard">
						...
						<div class="recard-bottom">
							<el-row class="recard-item" v-for="(item,index) in recardList" :key="index">
								...
								<el-col :span="6" :style="item.fangxiang=='买入'?'color:#67c23a':'color:#f56c6c'">{{item.fangxiang}}</el-col>
          ...
          <el-row class="list-item" v-for="(item,index) in changeList" :key="index">
            ...
            <el-col :span="3">
              <p
                style="color: #f07e1b;cursor: pointer;font-size: 12px"
                @click="checkDetail(item.trx_id)"
              >{{item.trx_id | sixLength}}</p>
            </el-col>
          </el-row>-->
					<el-table :data="changeList" height="" border style="width: 100%" :cell-style="cellStyle">
						...
						<el-table-column prop="jiaoyidui" label="交易对" width="118" align='center'>
						</el-table-column>
						<el-table-column prop="fangxiang" label="方向" align='center' width="120">
						</el-table-column>
						<el-table-column prop="price" label="成交价格" align='center' width="120">
						</el-table-column>
						<el-table-column prop="titt" label="成交数量(TITT)" align='center' width="120">
						</el-table-column>
						<el-table-column prop="eos" label="成交数量(EOS)" align='center' width="120">
						</el-table-column>
	...
	export default {
		...
		methods: {
				...
				if(type == "titt") {
					if(this.tittNum.trim() && this.tittNum >= 0.05) {
						...
						this.$router.push({
							...
							query: {
								titt: this.tittNum,
								eos: (this.tittNum / 200).toFixed(2),
								fangxiang: false
							}
						});
					}
					...
					if(this.eosNum.trim() && this.eosNum >= 10) {
						...
						this.$router.push({
							...
							query: {
								eos: this.eosNum,
								titt: (this.tittNum * 200).toFixed(2),
								fangxiang: true
							}
						});
					}
			...
			getChartData(val) {
				...
				this.$axios.get("displace/echars", {
					time: this.times
				}).then(res => {
					this.loading = false;
					if(res.data.sta == 1) {
			...
			lineChart() {
				...
				var option = {
					...
					series: [{
						...
						//          data: [ 
						//            [0.035598, 0.031671, 0.021888, 0.032998],
						//            [0.031042, 0.030996, 0.011111, 0.041111],
						//            [0.030057, 0.029944, 0.011111,0.031057],
						//            [0.030521, 0.028178, 0.018178, 0.031521]
						//          ]
						//        }
					]
				};
				...
				this.$axios
					...
					.then(res => {
						...
						if(res.data.data) {
							...
							//								res.data.data.list[i].trx_id = res.data.data.list[i].trx_id.substring(0, 5);
							//							}
