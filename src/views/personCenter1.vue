<template>
	<div class="center">
				...
				<div class="honggu1">
					<div class="honggu1_flex">
						<div class="left">
							...
							<!--<el-button
                size="small"
                @click="$router.push('/upEosUser')"
                v-if="Eos==''&& Eos== null&& Eos == undefined"
              >绑定eos账号</el-button>-->
		...
		<div class="orContinus" v-if='orContinusDialog'>
			<!--<el-dialog title="提示：您的铂金卡还有三天到期，是否选择继续持有,不选择默认为持有" :visible.sync="orContinusDialog" width="30%" center>
				<span slot="footer" class="dialog-footer">
    <el-button @click="continus">继续持有</el-button>
    <el-button type="primary" @click="stopHave">停止拥有</el-button>
  </span>
			</el-dialog>-->
			<div class="or_continus_title">
				<span>提示：您的</span>
				<span>铂金卡</span>
				<span>还有</span><span>三天</span><span>到期，是否选择继续持有,不选择默认为持有</span>
				<span>x</span>
			</div>
			<div class="continus_opt">
				<span class='look_detail'>查看详情</span><span class='continus_have'>继续持有</span>
			</div>
		</div>
	...
	export default {
		...
		methods: {
				...
				this.$axios
					...
					.then(res => {
						if(res.data.sta == 200) {
							...
							// this.modifyImg = val;
							// localStorage.setItem("modImg", val);
							// this.modifyImg = localStorage.getItem("modImg");
							this.getInfo();
							// this.noModify = false;
							// this.orModofySuccess = true;
						}
					});
			},
			selfDefine() {
			...
			beforeUpload(file) {
				...
				console.log(this.transferData.head_file, "aaa");
			},
			...
			getInfo() {
				this.$axios
					...
					.then(res => {
						...
						//						this.default_head = res.data.default.yuming+res.data.default.head_file
					});
			},
			//个人信息 （在线文档）
			getUserDetail() {
