<template>
	<div class="center">
		<div class="row">
                ...
                <span>KYC2认证</span>
						...
						<div class="info-right col-lg-7 col-md-6 col-sm-12">
							<div class="regist_phone">
								<span>注册手机号：</span>
								<span v-if='orbindPhone == true'>{{mobile}}</span>
								<el-button type="primary" v-else @click='goBindPhone' size="small" style="margin-left: 0;width:120px">绑定手机号</el-button>
							</div>
							...
							<el-button class="apply_honggu" type="primary" size="small" @click="friendHonggu">申请领取好友助力{{Number(honhguNum).toFixed(2)}}红股</el-button>
            ...
            <router-link to="/friendsList">好友列表>></router-link>
				...
				<div class="invite">
					<h4 class="myfriend">
            个人信息
            <router-link to="/contributionOrigin">个人贡献值来源>></router-link>
          </h4>
					<el-row class="row">
						<el-col :span="8" class="friend">
							<span>获得邀请奖励</span>
			...
			<div class="right col-lg-7 col-md-7 col-sm-12">
				<div class="bledge-container">
					<h4>会员卡信息<el-button style="float:right;margin: -10px 10px 0 0;" type="primary" @click='goCardDetail' round>会员卡详情</el-button></h4>
                ...
                <span style="font-size: 12px">（请点击会员卡的卡面，获取会员卡申请信息）</span>
							...
							<div class="vipKinds">
								<img v-for="item in cardList" :src="item.wenjian" @click="addJinka(item.classify,item.id,item.cardname)" alt />
							</div>
							<!--<h5>可参加抵押活动</h5>-->
		...
		<div class="orContinus zhezhao" v-if="orContinusZhezhao">
			<div class="popup1" v-if="moreCard">
				<span class="cancle" @click="cardCancle">x</span>
				<div class="or_continus_title">
					<span>提示：您有</span>
					<span>{{deallinecardNum}}</span>
					<span>张会员卡还有</span>
					<span>{{deallineday}}</span>
					<!--<span>三</span>-->
					<span>天到期，是否选择继续持有,不选择默认为持有</span>
				</div>
				<div class="continus_opt">
					<span class="look_detail" @click="lookCardDetail">查看详情</span>
					<span @click="stopHaveMoreCard" class="continus_have">停止持有</span>
				</div>
			</div>
			...
			<div class="popup" v-if="oneCard">
				...
				<div class="or_continus_title">
					...
					<span>{{cardType}}</span>
					<span>还有</span>
					<span>{{deallineday1}}</span>
					<span>天到期，是否选择继续持有,不选择默认为持有</span>
				</div>
				<div class="continus_opt">
					<span class="look_detail" @click="continusHave">继续持有</span>
					<span @click="stopHaveSingleCard" class="continus_have">停止持有</span>
				</div>
			</div>
		</div>
	</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			honggu() {
				...
				this.$axios
					...
					.then(res => {
						//						console.log(res)
						if(res.data.sta == 200) {
							that.honhguNum = res.data.count_bonus
			...
			showDefalutImg() {
				this.$axios.post("userinfo/defaultimg").then(res => {
					if(res.data.sta == 200) {
						var value = [];
			...
			isbindPhone() {
				...
				that.$axios
					...
					.then(res => {
						...
						if(res.data.sta == 200) {
							that.orbindPhone = true
						} else {
							that.orbindPhone = false
						}
					});
			},
			goBindPhone() {
				...
				if(this.checkLogin()) {
					if(url == "/hongguChange") {
						this.$router.push({
							path: url,
							query: {
								hongguyijiesuo: this.info.hongguyijiesuo
							}
						});
				...
				this.$axios
					...
					.then(res => {
						...
						for(var j in res.data.data) {
							deallineCardtotal =
								deallineCardtotal + Number(res.data.data[j].num);

						}
						//console.log(res)(deallineCardtotal, 'deallineCardtotal')
						//console.log(res)(res, 'ss')
						if(res.data.data[0].num == 0) {
							that.orContinusZhezhao = false;
							that.moreCard = false;
							that.oneCard = false;
						} else if(res.data.data[0].num == 1) { //1
							that.orContinusZhezhao = true;
							that.moreCard = false;
							that.oneCard = true;
							that.cardType = res.data.data[0].name;
							that.deallineday1 = res.data.data[0].days;
							that.cardId = res.data.data[0].id
						} else { //多
							that.orContinusZhezhao = true;
							that.moreCard = true;
							that.oneCard = false;
							that.deallinecardNum = res.data.data[0].num;
							that.deallineday = res.data.data[0].days;
							that.deallineNum = res.data.data[0].days;
							//							that.cardId = res.data.data[0].id
						}
						//						for(var j in res.data.data) {
						//							if(deallineCardtotal < 1) {
						//								//没有会员卡到期
						//								that.orContinusZhezhao = false;
						//								that.moreCard = false;
						//								that.oneCard = false;
						...
						//
						//								if(j == '三') {
						//									if(Number(res.data.data[j].num) == 0) {
						//										that.orContinusZhezhao = true;
						//										that.moreCard = false;
						//										//										that.oneCard = false;
						//										//										that.deallineday = j;
						...
						//									} else if(Number(res.data.data[j].num) == 1){
						//											that.orContinusZhezhao = true;
						//										that.oneCard = true;
						//										that.cardType = res.data.data[j].name;
						//										that.cardId = res.data.data[j].id
						//									}else if(Number(res.data.data[j].num) > 1) {
						//										that.orContinusZhezhao = true;
						//										//										that.oneCard = true;
						//										that.moreCard = true;
